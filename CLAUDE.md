...
- `cli/api/` — Subpackage for external API integrations.
  ...
  - `gemini_agent.py` — `PromptPattern(StrEnum)`: `SPELL` (typo-correction instructions) and `REWRITE` (query-expansion instructions). `get_gemini_client()` reads `GEMINI_API_KEY` from the environment (loaded via `python-dotenv`) and returns an authenticated `genai.Client`; raises `RuntimeError` if the key is absent. `enhance_query(query, method=None)` returns the original query unchanged when `method` is `None`; otherwise looks up the matching `PromptPattern` entry (raises `ValueError` for an unrecognised method), sends the query to Gemini, returns the corrected/rewritten text (or the original if the model returns nothing), prints the before/after, and logs token counts.
