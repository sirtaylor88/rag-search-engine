...
- `cli/semantic_search_cli.py` — Semantic search CLI entry point: registers `verify`, `verify_embeddings`, `embed_text`, `embed_query`, `search`, and `chunk` subcommands; dispatches to `VerifyCommand`, `VerifyEmbeddingsCommand` (both via `EmptyRequest`), `EmbedTextCommand`, `EmbedQueryCommand` (both via `TermRequest`), `SemanticSearchCommand` (via `SearchRequest`), and `ChunkCommand` (via `ChunkRequest`).
...
- `cli/schemas.py` — Pydantic `BaseModel` payload and request models. **Payload models**: `EmptyPayload` (no fields), `SearchPayload` (`query` min_length=1, `limit` ge=1 default `SEARCH_LIMIT`), `TermPayload` (`term` min_length=1), `ChunkPayload` (extends `TermPayload`, adds `chunk_size` ge=1 default `CHUNK_SIZE` and `overlap` ge=0 default `0`), `TermWithDocIDPayload` (adds `doc_id` ge=1), `BM25Payload` (adds `k1` gt=0, `b` ge=0 le=1). **Request envelope**: `Request[T]` generic `BaseModel` with a single `payload: T` field; concrete aliases `EmptyRequest`, `SearchRequest`, `TermRequest`, `ChunkRequest`, `TermWithDocIDRequest`, `BM25Request` narrow the type.
- `cli/commands/` — Command classes following an instance-based pattern.
  ...
  - `compute/chunk_command.py` — `ChunkCommand(TermCommand)`: registers `--chunk-size` (default `CHUNK_SIZE`) and `--overlap` (default `0`) optional args, splits the input text into overlapping word groups via `get_overlapping_chunks`, and prints each chunk with its index.
