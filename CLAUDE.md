...
# Verify multimodal image embedding (embed an image and print its shape)
uv run python cli/multimodal_search_cli.py verify_image_embedding <image_path>

# Image search: rank movies by cosine similarity between an image and text embeddings
...
- `cli/multimodal_search_cli.py` — Multimodal search CLI entry point: registers `verify_image_embedding` (via `VerifyImageEmbeddingCommand`) and `image_search` (via `ImageSearchCommand`) subcommands; each accepts `term` (positional, displayed as `image_path`); dispatches via `TermRequest(payload=TermPayload(term=args.term))`.
...
- `cli/commands/` — Command classes following an instance-based pattern.
  ...
  - `search/` — Subpackage for keyword, BM25, and semantic search commands.
    ...
    - `multimodal_search_commands.py` — `BaseMultimodalCommand(TermCommand)`: abstract base that registers `term` (displayed as `image_path`) and declares abstract `run()`. `VerifyImageEmbeddingCommand(BaseMultimodalCommand)`: `run()` calls `verify_image_embedding(request.payload.term)`. `ImageSearchCommand(BaseMultimodalCommand)`: `run()` loads movies, instantiates `MultimodalSearch(documents)`, calls `search_with_image(image_path)`, and prints ranked results as `"{idx}. {title} (similarity: {score:.3f})\n   {document[:100]}..."`.
...
- `cli/core/multimodal_search.py` — `MultimodalSearch(Singleton)`: singleton wrapper around a CLIP-based `SentenceTransformer` model (default `DEFAULT_MULTIMODAL_EMBEDDIND_MODEL = "clip-ViT-B-32"`); `__init__(documents, model_name)` loads the model and encodes all document titles+descriptions into `self.text_embeddings` on first instantiation; `embed_image(image_path)` opens the image via `PIL.Image.open`, encodes it with the CLIP model, and returns a 1-D `npt.NDArray[Any]` via `np.asarray`; `search_with_image(image_path, limit)` embeds the image, computes cosine similarity against each text embedding, and returns the top `limit` results as `list[dict]` with `id`, `score`, `title`, and `document` keys sorted by descending score. Module-level helper: `verify_image_embedding(image_path)` calls `load_movies()`, instantiates `MultimodalSearch(documents)`, embeds the image, and prints `"Embedding shape: {N} dimensions"`.
