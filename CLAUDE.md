...
- `cli/augmented_generation_cli.py` — RAG CLI entry point: registers a single `rag` subcommand via `RagCommand`; parses `query` (positional) and `--limit` (default `SEARCH_LIMIT`); dispatches to `RagCommand.run()` via `SearchRequest(payload=SearchPayload(...))`.
...
- `cli/api/` — Subpackage for external API integrations.
  ...
  - `gemini_agent.py` — `EnhancePromptPattern(StrEnum)`: `SPELL` (typo-correction), `REWRITE` (Google-style query rewrite), `EXPAND` (synonym/related-term expansion). `ReRankPromptPattern(StrEnum)`: `INDIVIDUAL` (0–10 relevance rating prompt for a single document), `BATCH` (prompt that asks the model to return a JSON-ordered list of doc IDs for a set of candidates), `EVALUATE` (0–3 relevance rating prompt for a list of results; returns a JSON score list). `AugmentedGenerationPromptPattern(StrEnum)`: `RAG` (system prompt for grounded answer generation, injects `{query}` and `{doc_input}`). `_display_token_usage(response)` private helper that logs prompt and response token counts at `INFO` level (called by all four public functions). `get_gemini_client()` reads `GEMINI_API_KEY` from the environment (loaded via `python-dotenv`) and returns an authenticated `genai.Client`; raises `RuntimeError` if the key is absent. `enhance_query(query, method=None)` returns the original query unchanged when `method` is `None`; otherwise looks up the matching `EnhancePromptPattern` entry (raises `ValueError` for an unrecognised method), sends the query to Gemini, returns the enhanced text (or the original if the model returns nothing), prints the before/after, and logs token counts. `rerank_query(query, doc_input, method=None)` returns `None` when `method` is `None`; otherwise looks up the matching `ReRankPromptPattern` entry (raises `ValueError` for an unrecognised method), sends query + document(s) to Gemini with a `GenerateContentConfig` that sets a hate-speech safety filter, and returns `response.text` as `Optional[str]` (raw model output, or `None` on empty response); logs token counts. `evaluate_query(query, results)` joins `results` with newlines, sends query + results to Gemini using `ReRankPromptPattern.EVALUATE`, and returns the raw JSON score string (`Optional[str]`); logs token counts. `augment_query(query, results)` joins `results` with newlines, sends query + results to Gemini using `AugmentedGenerationPromptPattern.RAG`, and returns `response.text` as `Optional[str]` (the grounded answer, or `None` when the model returns nothing); logs token counts.
