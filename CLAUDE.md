...
- `cli/semantic_search_cli.py` — Semantic search CLI entry point: registers `verify`, `verify_embeddings`, `embed_text`, `embed_query`, and `search` subcommands; dispatches to `VerifyCommand`, `VerifyEmbeddingsCommand` (both via `EmptyRequest`), `EmbedTextCommand`, `EmbedQueryCommand` (both via `TermRequest`), and `SemanticSearchCommand` (via `SearchRequest`).
...
- `cli/constants.py` — Project-wide constants: `BM25_K1` (default BM25 saturation parameter, `1.5`), `BM25_B` (default length normalization parameter, `0.75`), `CACHE_DIR` (cache directory name, `"cache"`), `CACHE_DIR_PATH` (`Path(CACHE_DIR)`), `SEARCH_LIMIT` (default result count, `5`).
