...
- `cli/api/` — Subpackage for external API integrations.
  - `gemini_agent.py` — `get_gemini_client()` reads `GEMINI_API_KEY` from the environment (loaded via `python-dotenv`) and returns an authenticated `genai.Client`; raises `RuntimeError` if the key is absent. `enhance_query(query, method)` sends the query to Gemini with a spell-correction prompt, returns the corrected text (or the original if the model returns nothing), prints the before/after, and logs token counts.
