...
- `cli/commands/` — Command classes following an instance-based pattern.
  ...
  - `search/` — Subpackage for keyword, BM25, and semantic search commands.
    ...
    - `hybrid_search_commands.py` — `BaseHybridSearchCommand(BaseSearchCommand, Generic[P])`: generic abstract base (`P bound SearchPayload`) whose `run()` calls `_get_query(payload)` to resolve the final query string, loads movies, calls abstract `_search(hs, payload, query)`, then prints either a re-ranking banner (`"Re-ranking top N results using <method> method..."`) or a plain results banner depending on whether `payload.rerank_method` is set; iterates results via abstract `_format_scores(result)`, printing `Re-rank Score: {new_score:.3f}/10` for `"individual"`, `Re-rank Rank: {idx}` for `"batch"`, or `Cross Encoder Score: {score}` for `"cross_encoder"`. Non-abstract `_get_query(payload)` defaults to `payload.query`. `WeightedSearchCommand(BaseHybridSearchCommand[WeightedSearchPayload])`: registers `--alpha`; `_search(hs, payload, query)` calls `weighted_search(query, alpha, limit)`; `_format_scores` returns hybrid, BM25, and semantic scores. `RRFSearchCommand(BaseHybridSearchCommand[RRFSearchPayload])`: registers `--k`, `--enhance` (choices via `get_args(EnhanceMethod)`: `"spell"`, `"rewrite"`, `"expand"`), and `--rerank-method` (choices via `get_args(ReRankeMethod)`: `"individual"`, `"batch"`, `"cross_encoder"`); `_get_query` delegates to `enhance_query`; `_search(hs, payload, query)` — without `--rerank-method` calls `rrf_search(query, k, limit)` directly; otherwise fetches `5 × limit` candidates then re-ranks: `individual` calls `rerank_query` per result (with `sleep(3)` between calls) and sorts by float score; `batch` builds a single `doc_input` string, calls `rerank_query` once, parses the JSON ID list (falls back to original RRF order on empty/None response); `cross_encoder` scores all pairs via `CrossEncoder(DEFAULT_CROSS_ENCODER_MODEL).predict()` and sorts descending; `_format_scores` returns RRF score, BM25 rank, and semantic rank.
...
- `cli/constants.py` — Project-wide constants: `BM25_K1` (default BM25 saturation parameter, `1.5`), `BM25_B` (default length normalization parameter, `0.75`), `CACHE_DIR` (cache directory name, `"cache"`), `CACHE_DIR_PATH` (`Path(CACHE_DIR)`), `SEARCH_LIMIT` (default result count, `5`), `CHUNKED_SEARCH_LIMIT` (result count for chunked searches, `10`), `DEFAULT_ALPHA` (default BM25 weight for hybrid scoring, `0.5`), `DEFAULT_K` (default RRF smoothing constant, `60`), `CHUNK_SIZE` (default words per chunk, `200`), `SEMANTIC_CHUNK_SIZE` (default sentences per semantic chunk, `4`), `SENTENCE_SPLIT_PATTERN` (regex for sentence boundary splitting, `r"(?<=[.!?])\s+"`), `DEFAULT_EMBEDDING_MODEL` (sentence-transformers model name, `"all-MiniLM-L6-v2"`), `SCORE_PRECISION` (decimal places for displayed scores, `4`), `GEMINI_MODEL` (Gemini model ID for query enhancement, `"gemma-4-31b-it"`), `DEFAULT_ENHANCE_METHOD` (default query enhancement method label, `"spell"`), `DEFAULT_CROSS_ENCODER_MODEL` (cross-encoder model ID for local re-ranking, `"cross-encoder/ms-marco-TinyBERT-L2-v2"`).
- `cli/api/` — Subpackage for external API integrations.
  - `__init__.py` — Package marker; no public exports.
  - `gemini_agent.py` — `EnhancePromptPattern(StrEnum)`: `SPELL` (typo-correction), `REWRITE` (Google-style query rewrite), `EXPAND` (synonym/related-term expansion). `ReRankPromptPattern(StrEnum)`: `INDIVIDUAL` (0–10 relevance rating prompt for a single document), `BATCH` (prompt that asks the model to return a JSON-ordered list of doc IDs for a set of candidates). `get_gemini_client()` reads `GEMINI_API_KEY` from the environment (loaded via `python-dotenv`) and returns an authenticated `genai.Client`; raises `RuntimeError` if the key is absent. `enhance_query(query, method=None)` returns the original query unchanged when `method` is `None`; otherwise looks up the matching `EnhancePromptPattern` entry (raises `ValueError` for an unrecognised method), sends the query to Gemini, returns the enhanced text (or the original if the model returns nothing), prints the before/after, and logs token counts. `rerank_query(query, doc_input, method=None)` returns `None` when `method` is `None`; otherwise looks up the matching `ReRankPromptPattern` entry (raises `ValueError` for an unrecognised method), sends query + document(s) to Gemini with a `GenerateContentConfig` that sets a hate-speech safety filter, and returns `response.text` as `Optional[str]` (raw model output, or `None` on empty response); logs token counts.
