...
- `cli/augmented_generation_cli.py` — Augmented generation CLI entry point: registers `rag` and `summarize` subcommands via `RagCommand` and `SummarizeCommand`; each accepts `query` (positional) and `--limit` (default `SEARCH_LIMIT`); dispatches via `SearchRequest(payload=SearchPayload(...))`.
