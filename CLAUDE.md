...
- `cli/commands/` — Command classes following an instance-based pattern.
  - `base.py` — `BaseCommand[RequestT]` abstract base with `__init__(parser)`, abstract `add_arguments(parser)`, abstract `run(request)`, concrete `load_cache()` (shared OSError handling), and `inverted_index` property that returns the `InvertedIndex` singleton. Pydantic request models (each a `BaseModel` subclass with field validation): `SearchRequest` (`query` min_length=1, `limit` ge=1), `TermRequest` (`term` min_length=1), `TermWithDocIDRequest` (adds `doc_id` ge=1), `BM25Request` (adds `k1` gt=0, `b` ge=0 le=1). Concrete bases: `TermCommand` (registers `term` positional arg), `BaseSearchCommand` (registers `query` positional arg and `--limit` optional arg).
