...
- `cli/commands/` — Command classes following an instance-based pattern.
  ...
  - `compute/chunk_command.py` — `ChunkCommand(TermCommand)`: registers `--chunk-size` optional arg (default `CHUNK_SIZE`), splits the input text into word groups via `itertools.batched`, and prints each chunk with its index.
