...
- `cli/commands/` — Command classes following an instance-based pattern.
  ...
  - `search/` — Subpackage for keyword, BM25, and semantic search commands.
    ...
    - `hybrid_search_commands.py` — `BaseHybridSearchCommand(BaseSearchCommand, Generic[P])`: generic abstract base (`P bound SearchPayload`) whose `run()` calls `_get_query(payload)` to resolve the final query string, loads movies, calls abstract `_search(hs, payload, query)`, prints the query banner, and iterates results via abstract `_format_scores(result)`; also prints `Re-rank Score: {new_score:.3f}/10` when `payload.rerank_method == "individual"` or `Re-rank Rank: {idx}` when `payload.rerank_method == "batch"`. Non-abstract `_get_query(payload)` defaults to `payload.query`. `WeightedSearchCommand(BaseHybridSearchCommand[WeightedSearchPayload])`: registers `--alpha`; `_search(hs, payload, query)` calls `weighted_search(query, alpha, limit)`; `_format_scores` returns hybrid, BM25, and semantic scores. `RRFSearchCommand(BaseHybridSearchCommand[RRFSearchPayload])`: registers `--k`, `--enhance` (choices via `get_args(EnhanceMethod)`: `"spell"`, `"rewrite"`, `"expand"`), and `--rerank-method` (choices via `get_args(ReRankeMethod)`: `"individual"`, `"batch"`); `_get_query` delegates to `enhance_query`; `_search(hs, payload, query)` — without `--rerank-method` calls `rrf_search(query, k, limit)`; with `--rerank-method individual` fetches `5 × limit` candidates, calls `rerank_query` per result (with `sleep(3)` between calls), converts string response to float, sorts descending by `new_score`, and returns the top `limit`; with `--rerank-method batch` fetches `5 × limit` candidates, concatenates all as a single `doc_input`, calls `rerank_query` once, parses the JSON list of IDs (falls back to original RRF order on empty/None response), sorts by that order, and returns the top `limit`; `_format_scores` returns RRF score, BM25 rank, and semantic rank.
