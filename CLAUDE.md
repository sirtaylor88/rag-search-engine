...
- `cli/augmented_generation_cli.py` — Augmented generation CLI entry point: registers `rag`, `summarize`, and `citations` subcommands via `RagCommand`, `SummarizeCommand`, and `CitationsCommand`; each accepts `query` (positional) and `--limit` (default `SEARCH_LIMIT`); dispatches via `SearchRequest(payload=SearchPayload(...))`.
