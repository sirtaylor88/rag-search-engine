...
- `cli/commands/` — Command classes following an instance-based pattern.
  ...
  - `search/` — Subpackage for keyword, BM25, and semantic search commands.
    ...
    - `hybrid_search_commands.py` — `WeightedSearchCommand(BaseSearchCommand)`: registers `query`, `--limit`, and `--alpha`; `run()` loads movies, instantiates `HybridSearch`, calls `weighted_search(query, alpha, limit)`, and prints ranked results with hybrid, BM25, and semantic scores. `RRFSearchCommand(BaseSearchCommand)`: registers `query`, `--limit`, and `--k`; `run()` loads movies, instantiates `HybridSearch`, calls `rrf_search(query, k, limit)`, and prints ranked results with RRF score, BM25 rank, and semantic rank.
