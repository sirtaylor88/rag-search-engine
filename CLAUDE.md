...
- `cli/hybrid_search_cli.py` — Hybrid search CLI entry point: registers `normalize`; dispatches to `NormalizeCommand` (via `ScoreListRequest`).
...
- `cli/schemas/` — Pydantic `BaseModel` payload and request models, split into two submodules; `cli/schemas/__init__.py` re-exports everything so all `from cli.schemas import …` calls work unchanged.
  ...
  - `requests.py` — **Request envelope**: `Request[T]` generic `BaseModel` with a single `payload: T` field; concrete aliases `EmptyRequest`, `SearchRequest`, `TermRequest`, `ChunkRequest`, `SemanticChunkRequest`, `TermWithDocIDRequest`, `BM25Request`, `ScoreListRequest` narrow the type.
- `cli/commands/` — Command classes following an instance-based pattern.
  ...
  - `search/` — Subpackage for keyword, BM25, and semantic search commands.
    ...
    - `semantic_search_command.py` — `BaseSemanticSearchCommand(BaseSearchCommand)`: abstract base whose `run()` loads movies, calls abstract `_prepare(documents)`, prints the query banner, calls abstract `_search(query, limit)`, and prints results via abstract `_get_excerpt(result)`. `SemanticSearchCommand(BaseSemanticSearchCommand)`: `_prepare` loads corpus embeddings; `_search` calls `SemanticSearch.search()`; `_get_excerpt` returns `result["description"]`. `SearchChunkedCommand(BaseSemanticSearchCommand)`: `_prepare` loads chunk embeddings; `_search` calls `ChunkedSemanticSearch.search_chunks()`; `_get_excerpt` returns `result["document"]`.
