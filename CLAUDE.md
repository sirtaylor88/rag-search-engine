...
- `cli/multimodal_search_cli.py` — Multimodal search CLI entry point: registers `verify_image_embedding` subcommand via `VerifyImageEmbeddingCommand`; accepts `term` (positional, displayed as `image_path`); dispatches via `TermRequest(payload=TermPayload(term=args.term))`.
...
- `cli/core/multimodal_search.py` — `MultimodalSearch(Singleton)`: singleton wrapper around a CLIP-based `SentenceTransformer` model (default `DEFAULT_MULTIMODAL_EMBEDDIND_MODEL = "clip-ViT-B-32"`); `__init__` loads the model on first instantiation; `embed_image(file_path)` opens the image via `PIL.Image.open`, encodes it with the CLIP model, and returns a 1-D `npt.NDArray[Any]` via `np.asarray`. Module-level helper: `verify_image_embedding(file_path)` instantiates `MultimodalSearch`, embeds the image, and prints `"Embedding shape: {N} dimensions"`.
