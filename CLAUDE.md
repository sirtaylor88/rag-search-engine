...
- `cli/commands/` — Command classes following an instance-based pattern.
  - `base.py` — `BaseCommand[RequestT]` abstract base with `__init__(parser)`, abstract `add_arguments(parser)`, abstract `run(request)`, concrete `load_cache()` (shared OSError handling), and `inverted_index` property that returns the `InvertedIndex` singleton. Request dataclasses: `SearchRequest` (`query`, `limit`), `TermRequest` (`term`), `TermWithDocIDRequest` (`doc_id`, `term`), `BM25Request` (adds `k1`, `b`). Concrete bases: `TermCommand` (registers `term` positional arg), `BaseSearchCommand` (registers `query` positional arg and `--limit` optional arg).
