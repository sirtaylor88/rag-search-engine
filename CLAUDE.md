...
- `cli/commands/` — Command classes following an instance-based pattern.
  ...
  - `compute/chunk_commands.py` — `BaseChunkCommand(TermCommand)`: abstract base that registers the shared `--overlap` (default `0`) argument. `ChunkCommand(BaseChunkCommand)`: registers `--chunk-size` (default `CHUNK_SIZE`), splits the input text into overlapping word groups via `get_overlapping_chunks`, and prints each chunk with its index. `SemanticChunkCommand(BaseChunkCommand)`: registers `--max-chunk-size` (default `SEMANTIC_CHUNK_SIZE`), splits the input text into sentence groups using `SPLIT_PATTERN` via `re.split`, then chunks and prints each group with its index.
