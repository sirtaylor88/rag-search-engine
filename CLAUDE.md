...
- `cli/commands/` — Command classes following an instance-based pattern.
  ...
  - `search/` — Subpackage for keyword, BM25, and semantic search commands.
    ...
    - `hybrid_search_commands.py` — `BaseHybridSearchCommand(BaseSearchCommand, Generic[P])`: generic abstract base (`P bound SearchPayload`) whose `run()` calls `_get_query(payload)` to resolve the final query string, loads movies, calls abstract `_search(hs, payload, query)`, prints the query banner, and iterates results via abstract `_format_scores(result)`; also prints `Re-rank Score: {new_score:.3f}/10` when `payload.rerank_method == "individual"`. Non-abstract `_get_query(payload)` defaults to `payload.query`. `WeightedSearchCommand(BaseHybridSearchCommand[WeightedSearchPayload])`: registers `--alpha`; `_search(hs, payload, query)` calls `weighted_search(query, alpha, limit)`; `_format_scores` returns hybrid, BM25, and semantic scores. `RRFSearchCommand(BaseHybridSearchCommand[RRFSearchPayload])`: registers `--k`, `--enhance` (choices via `get_args(EnhanceMethod)`: `"spell"`, `"rewrite"`, `"expand"`), and `--rerank-method` (choices via `get_args(ReRankeMethod)`: `"individual"`); `_get_query` delegates to `enhance_query`; `_search(hs, payload, query)` — without `--rerank-method` calls `rrf_search(query, k, limit)`; with `--rerank-method individual` fetches `5 × limit` candidates, calls `rerank_query` per result (with `sleep(3)` between calls), sorts descending by `new_score`, and returns the top `limit`; `_format_scores` returns RRF score, BM25 rank, and semantic rank.
...
- `cli/api/` — Subpackage for external API integrations.
  ...
  - `gemini_agent.py` — `EnhancePromptPattern(StrEnum)`: `SPELL` (typo-correction), `REWRITE` (Google-style query rewrite), `EXPAND` (synonym/related-term expansion). `ReRankPromptPattern(StrEnum)`: `INDIVIDUAL` (0–10 relevance rating prompt for a single document). `get_gemini_client()` reads `GEMINI_API_KEY` from the environment (loaded via `python-dotenv`) and returns an authenticated `genai.Client`; raises `RuntimeError` if the key is absent. `enhance_query(query, method=None)` returns the original query unchanged when `method` is `None`; otherwise looks up the matching `EnhancePromptPattern` entry (raises `ValueError` for an unrecognised method), sends the query to Gemini, returns the enhanced text (or the original if the model returns nothing), prints the before/after, and logs token counts. `rerank_query(query, doc_input, method=None)` returns `0.0` when `method` is `None`; otherwise looks up the matching `ReRankPromptPattern` entry (raises `ValueError` for an unrecognised method), sends query + document to Gemini with a `GenerateContentConfig` that sets a hate-speech safety filter, and returns `float(response.text)` (or `0.0` on empty response); logs token counts.
