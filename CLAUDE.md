...
- `cli/semantic_search_cli.py` — Semantic search CLI entry point: registers the `verify` subcommand and dispatches it to `VerifyCommand` via `EmptyRequest`.
...
- `cli/core/semantic_search.py` — `SemanticSearch` class wrapping `SentenceTransformer("all-MiniLM-L6-v2")`; `verify_model()` instantiates it and prints model info.
